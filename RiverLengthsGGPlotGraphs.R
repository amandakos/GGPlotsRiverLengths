#Always stat with the basics! Make sure to call up ggplot2 and access your data frame.

library(ggplot2)
View(rivers)

#Here we will create a usable data frame out of the list of numbers (river lenghts) in the rivers data frame.

rr = data.frame(rivers)
View(rr)

#Next set up the basic ggplot to run a histogram to view the data.
...
ggplot (rr, aes (x = rivers)) + geom_histogram(bins = 20, color="blue4", fill="blue1") +
  ggtitle("River Length Comparison") + xlab("Length") + ylab ("Number of Rivers")
...
#Visually I am seeing several outliers. But let's check the mathematical way for fun.
#Math is fun!
